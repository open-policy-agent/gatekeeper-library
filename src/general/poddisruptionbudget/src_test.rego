...
pdb_min_available(min_available) := output if {
  output := {
...
pdb_max_unavailable(max_unavailable) := output if {
  output := {
    "apiVersion": "policy/v1",
    "kind": "PodDisruptionBudget",
    "metadata": {
...
input_pdb_max_unavailable(max_unavailable) := output if {
  output := {
...
input_deployment(replicas) := output if {
  output := {
