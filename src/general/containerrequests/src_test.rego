...
test_no_ram_request {
    input := {"review": review([{"name": "a", "resources": {"requests": {"cpu": 1}}}]), "parameters": {"memory": "2", "cpu": "4"}}
    results := violation with input as input
    count(results) == 1
}
test_no_cpu_request {
    input := {"review": review([{"name": "a", "resources": {"requests": {"memory": 1}}}]), "parameters": {"memory": "2", "cpu": "4"}}
    results := violation with input as input
    count(results) == 1
}
test_no_request {
