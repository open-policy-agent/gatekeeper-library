---
id: gpuworkloadresources
title: GPU Workload Resources
---
...
metadata:
  ...
  annotations:
    ...
    description: >-
      ...
      a CPU request. Containers that request GPUs must also set matching GPU
      requests and limits. This keeps GPU workloads schedulable and predictable
      while still allowing CPU burst above request.
spec:
  crd:
    spec:
      ...
      validation:
        openAPIV3Schema:
          type: object
          description: >-
            Enforces memory request/limit alignment and cpu requests for GPU pods,
            plus matching gpu requests and limits on GPU containers.
          properties:
            exemptImages:
              description: >-
...
### Usage
```shell
kubectl apply -f https://raw.githubusercontent.com/open-policy-agent/gatekeeper-library/master/library/general/gpuworkloadresources/template.yaml
```
## Examples
<details>
<summary>gpu-pod-compliant</summary>
...
```shell
kubectl apply -f https://raw.githubusercontent.com/open-policy-agent/gatekeeper-library/master/library/general/gpuworkloadresources/samples/gpu-pod-compliant/constraint.yaml
```
...
```shell
kubectl apply -f https://raw.githubusercontent.com/open-policy-agent/gatekeeper-library/master/library/general/gpuworkloadresources/samples/gpu-pod-compliant/example_allowed.yaml
```
...
<summary>gpu-pod-memory-mismatch</summary>
...
```shell
kubectl apply -f https://raw.githubusercontent.com/open-policy-agent/gatekeeper-library/master/library/general/gpuworkloadresources/samples/gpu-pod-memory-mismatch/constraint.yaml
```
...
```shell
kubectl apply -f https://raw.githubusercontent.com/open-policy-agent/gatekeeper-library/master/library/general/gpuworkloadresources/samples/gpu-pod-memory-mismatch/example_disallowed.yaml
```
...
<summary>gpu-pod-cpu-request-missing</summary>
...
```shell
kubectl apply -f https://raw.githubusercontent.com/open-policy-agent/gatekeeper-library/master/library/general/gpuworkloadresources/samples/gpu-pod-cpu-request-missing/constraint.yaml
```
...
kind: Pod
metadata:
  name: gpu-pod-cpu-request-missing
spec:
  containers:
    - name: training
      image: nvidia/cuda:12.0-runtime
...
```shell
kubectl apply -f https://raw.githubusercontent.com/open-policy-agent/gatekeeper-library/master/library/general/gpuworkloadresources/samples/gpu-pod-cpu-request-missing/example_disallowed.yaml
```
...
```shell
kubectl apply -f https://raw.githubusercontent.com/open-policy-agent/gatekeeper-library/master/library/general/gpuworkloadresources/samples/non-gpu-pod/constraint.yaml
```
...
```shell
kubectl apply -f https://raw.githubusercontent.com/open-policy-agent/gatekeeper-library/master/library/general/gpuworkloadresources/samples/non-gpu-pod/example_allowed.yaml
```
